...
- `equals` - Exact match
- `not_equals` - Not equal (negated exact match)
...
**For complex multi-condition rules:**
Use the `payload` parameter with raw Targeting proto JSON. See the schema documentation below.

## Confidence Targeting Schema

The Targeting proto defines complex targeting rules for feature flags.
...
Each criterion targets an attribute with a rule:
...
**RangeRule** (gt, gte, lt, lte):
```json
"rangeRule": {
  "startInclusive": { "numberValue": 18 },
  "endExclusive": { "numberValue": 65 }
}
```
Options: startInclusive, startExclusive, endInclusive, endExclusive

**StartsWithRule**:
```json
"startsWithRule": { "value": "prefix-" }
```

**EndsWithRule**:
```json
"endsWithRule": { "value": "@spotify.com" }
```
...
#### Single condition (plan in [pro, enterprise]):
```json
{
  "criteria": {
    "ref-0": {
...
#### Multiple conditions with AND (plan=pro AND country=US):
```json
{
  "criteria": {
    "ref-0": {
...
#### NOT condition (plan NOT IN [free]):
```json
{
  "criteria": {
    "ref-0": {
      "attribute": {
        "attributeName": "plan",
        "setRule": { "values": [{ "stringValue": "free" }] }
      }
    }
  },
  "expression": {
...
#### Complex: (plan=pro AND age>=18) OR email ends with @spotify.com
```json
{
  "criteria": {
    "ref-0": {
    ...
    "ref-1": {
      "attribute": {
        "attributeName": "age",
        "rangeRule": { "startInclusive": { "numberValue": 18 } }
      }
    },
    "ref-2": {
      "attribute": {
